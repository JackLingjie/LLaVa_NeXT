...
# 提取路径中的文件名  
file_names = [os.path.basename(path) for path in json_paths]  
  
# 创建short_names到file_names的映射  
name_to_file_map = dict(zip(short_names, file_names))  
  
# 将字典存储到JSON文件中  
with open('process_data/convert/name_to_file_map.json', 'w') as json_file:  
    json.dump(name_to_file_map, json_file, indent=4)  
  
print("字典已成功存储到 name_to_file_map.json 文件中。")  