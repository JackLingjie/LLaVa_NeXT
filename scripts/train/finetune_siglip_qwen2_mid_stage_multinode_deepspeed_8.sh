...
MID_RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-mid_stage_${MAX_MODEL_LEN}_multinode_deepspeed_8nodes"
echo "MID_RUN_NAME: ${MID_RUN_NAME}"

CKPT_PATH=$LLM_VERSION # this could also be the previous stage checkpoint
NUM_GPUS=8

deepspeed --num_gpus 8 --num_nodes 8 --hostfile scripts/train/config/hostfile8 \
